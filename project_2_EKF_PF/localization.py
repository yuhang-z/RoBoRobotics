...
class Observation(object):
    ...
    def __init__(self, landmark_id, rng):
        self.landmark_id = landmark_id     # rng is the distance between landmark and the robtot 
        self.rng = float(rng)
...
def dynamics_model(state, control, dt):
    # TODO: fill this
    x = state.x + control.vx * dt   # usually the state and control is at the previous step
    y = state.y + control.vy * dt
    expected_state = State(x, y)     # MUST CHANGE THESE VALUES
    return expected_state

def measurement_model(state, landmark):
    # TODO: fill this
    diff_x = state.x - landmark.x
    diff_y = state.y - landmark.y 
    distance = np.sqrt(diff_x ** 2 + diff_y ** 2)          
    expected_observation = Observation(landmark.id, distance)                
...
class LocalizationWithRangeMeasurements(object):
    ...
    def dynamics_cost(self, state_curr, state_prev, control_prev):
        ...
        expected_state = dynamics_model(state_prev, control_prev, self.dt)
        return np.linalg.norm(expected_state.diff(state_curr)) *(1/self.dynamics_noise_std_dev)
        # diff = sqrt((state_curr.x - expected_state.x)**2 - (state_curr.y - expected_state.y)**2) * self.dynamics_noise_std_dev  
        # return diff
    ...
    def measurement_cost(self, state, landmark, observation):
        ...
        # measurement model
        # I.e. returns a vector
        predicted_landmark_dis = np.sqrt((landmark.x - state.x)**2 + (landmark.y - state.y)**2)
        diff = observation.rng - predicted_landmark_dis
        return diff
    ...
    def cost_function(self, x):
        ...
        predicted_state = [State(x[State.dim*t], x[State.dim*t + 1]) for t in range(T)]

        # TODO: Extend F by the deviation of the estimated first state (x[0], x[1])
        # from the given first state self.init_state
        deviation_from_init_state = np.array([sqrt((self.init_state.x - x[0])**2 + (self.init_state.y - x[1])**2)])                             # MUST CHANGE THIS VALUE!
        ...
        for t in xrange(1, T):
            ...
            control = self.controls_across_time[t-1]
            dc = np.array([self.dynamics_cost(predicted_state[t], predicted_state[t-1], control)])
        ...
        for t in xrange(self.num_timesteps):
            ...
            # dc = np.array(([x[t] - observations_across_time[t].x]), (x[t+1] - observations_across_time[t].y))
            for observation in self.observations_across_time[t]:
                landmark = self.landmarks[observation.landmark_id]
                mc = np.array([self.measurement_cost(predicted_state[t], landmark, observation)])
