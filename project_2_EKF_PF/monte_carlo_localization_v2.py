...
class ParticleFilter(object):
    ...
    def handle_observation(self, laser_scan, dt):
        ...
        for p in self.particles:
            self.predict_particle_odometry(p)   #p_motion is the paricle's relative motion 
        ...
        self.weights = self.exp_normalize(np.negative(errors)) 
        
        N_eff = 1/np.sum([w**2 for w in self.weights])
    ...
    def resample(self):
        ...
        N = len(self.particles)   # number of the paticles 
        ...
        #cum_sum[-1] = 1
        #indexes = np.searchsorted(cum_sum, random(n))
    ...
    def simulate_laser_scan_for_particle(self, x, y, yaw_in_map, angles, min_range, max_range):
        ...
        for a in angles:
            abs_angle = yaw_in_map + a #1

            delta_x = cos(abs_angle) * range_step #2
            delta_y = sin(abs_angle) * range_step

            x_new = x + cos(abs_angle) * min_range
            y_new = y + sin(abs_angle) * min_range 

            p_range = max_range

            for i in range(int((max_range-min_range)/range_step)):  # meake sure its withint the max_range 
                point_x = x_new + delta_x * i
                point_y = y_new + delta_y * i 
                if  ((xmin < point_x < xmax) and (ymin < point_y < ymax)):
                    row, col = self.metric_to_grid_coords(point_x, point_y)
                    if not self.grid_bin[row, col]: 
                        p_range = sqrt((point_x - x)**2 + (point_y - y)**2 )   #p_range is the range of the examined points
                        break 
    ...
    def get_prediction_error_squared(self, laser_scan_msg, particle):
        ...
        sub_ranges, sub_angles = self.subsample_laser_scan(laser_scan_msg)
        # TODO: simulate a laser scan using one of the methods of this class
        stimu_laser_range = self.simulate_laser_scan_for_particle(particle.x, particle.y, particle.theta, sub_angles, self.laser_min_range, self.laser_max_range)
        # TODO: compute the difference bwteen predicted ranges and actual ranges
        # Take the squared norm of that difference
        error_diff = np.array(stimu_laser_range) - np.array(sub_ranges)
        return np.dot(error_diff, error_diff)
