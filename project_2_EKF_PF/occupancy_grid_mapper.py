...
class HuskyMapper:
    ...
    def odometry_callback(self, msg):
        ...
        # 
        odometry_orientation = self.odometry.pose.pose.orientation
        self.q_map_baselink = np.array([odometry_orientation.x, odometry_orientation.y, odometry_orientation.z, odometry_orientation.w])
        ...
        #       multiplication, which is not usually what you want
        #
        self.p_map_baselaser = np.matmul(self.R_map_baselink, self.p_baselink_baselaser) + self.p_map_baselink
        self.mutex.release()
    ...
    def is_in_field_of_view(self, robot_row, robot_col, robot_theta, row, col):
        ...
        #
        diff_x = self.ogm.meters_per_cell * (col - robot_col)
        diff_y = self.ogm.meters_per_cell * (row - robot_row)
        distance = sqrt(diff_x **2 + diff_y ** 2)
        angle_p = atan2(diff_y, diff_x)
        angle_diff = atan2(sin(angle_p - robot_theta), cos(angle_p - robot_theta))
        if distance > self.max_laser_range or self.min_laser_angle > angle_diff > self.max_laser_angle:
            return False 
        return True  
    ...
    def inverse_measurement_model(self, row, col, robot_row, robot_col, robot_theta_in_map, beam_ranges, beam_angles):
        ...
        angle_p = atan2(diff_y, diff_x)
        diff_angle = atan2(sin(angle_p - robot_theta_in_map), cos(angle_p - robot_theta_in_map))
